...
func CreatePaiement(db *sql.DB, p *model.Paiement) (int64, error) {
	result, err := db.Exec("INSERT INTO paiement (id_annonce, id_service, id_payeur, id_receveur, montant, date_paiement) VALUES (?, ?, ?, ?, ?, ?)", p.IDAnnonce, p.IDService, p.IDPayeur, p.IDReceveur, p.Montant, p.DatePaiement)
	if err != nil {
	...
	if err != nil {
		log.Printf("Erreur lors de la récupération de l'ID du paiement créé: %v", err)
		return 0, err
	}
	p.ID = int(id)
