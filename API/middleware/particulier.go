...
func GetParticulierMiddleware(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(particuliers) == 0 {
			c.JSON(http.StatusNotFound, gin.H{"error": "Aucun particulier trouvé"})
			c.Abort()
			return
		}
		c.Next()
...
func GetParticulierByIDMiddleware(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "ID du particulier manquant"})
			c.Abort()
			return
		}
...
func CreateParticulierMiddleware(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if nom == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Nom du particulier manquant"})
			c.Abort()
			return
		}

		if prenom == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Prénom du particulier manquant"})
			c.Abort()
			return
		}
		...
		for _, c2 := range nom {
			if (c2 >= '0' && c2 <= '9') || (c2 >= 33 && c2 <= 47) || (c2 >= 58 && c2 <= 64) || (c2 >= 91 && c2 <= 96) || (c2 >= 123 && c2 <= 126) {
				c.JSON(http.StatusBadRequest, gin.H{"error": "Nom du particulier invalide"})
				c.Abort()
				return
			}
		}
		...
		for _, c2 := range prenom {
			if (c2 >= '0' && c2 <= '9') || (c2 >= 33 && c2 <= 47) || (c2 >= 58 && c2 <= 64) || (c2 >= 91 && c2 <= 96) || (c2 >= 123 && c2 <= 126) {
				c.JSON(http.StatusBadRequest, gin.H{"error": "Prénom du particulier invalide"})
				c.Abort()
				return
			}
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur vérification email"})
			c.Abort()
			return
		}
