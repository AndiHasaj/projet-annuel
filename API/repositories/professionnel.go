...
func CreateProfessionnel(db *sql.DB, p *model.Professionnel) (int64, error) {
	result, err := db.Exec("INSERT INTO professionnel (nom_entreprise, email, mot_de_passe, numero_siret, telephone, site_web, adresse, code_postal, ville) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
		p.NomEntreprise, p.Email, p.MotDePasse, p.NumeroSiret, p.Telephone, p.SiteWeb, p.Adresse, p.CodePostal, p.Ville)
	if err != nil {
